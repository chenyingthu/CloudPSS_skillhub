#!/usr/bin/env python3
"""
model_validator Skill - Integration Tests

Tests for model_validator skill with real CloudPSS API.
"""
...
class TestModelValidatorSkillIntegration:
    """Integration tests for model_validator skill"""

    def setup_method(self):
        """Setup for each test"""
        self.skill = ModelValidatorSkill()
    ...
    def test_skill_registration(self):
        ...
        skill = get_skill("model_validator")
        assert skill is not None
        assert skill.name == "model_validator"
    ...
    def test_integration_run_model_validator(self, auth_token):
        """Test running model validator on IEEE3 model"""
        server: "internal"
        config = {
            "skill": "model_validator",
            "auth": {"token": auth_token, "server": "internal"},
