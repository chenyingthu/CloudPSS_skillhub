...
class TestBatchPowerFlowUnit:
    ...
    def test_partial_fetch_failure_returns_failed_skill_result(self, mock_model_class, mock_set_token):
        ...
        ok_job.result.getBuses.return_value = [{"data": {"columns": [{"name": "Vm", "data": [1.0]}]}}]
        ok_job.result.getBranches.return_value = [{"data": {"columns": [{"name": "Branch", "data": ["line-1"]}]}}]
        ...
        config = {
            ...
            "output": {"path": "/tmp", "aggregate": False, "timestamp": False, "prefix": "batch_powerflow_unit"},
        }
        ...
        bad = next(item for item in result.data["results"] if item["model_name"] == "BadModel")
