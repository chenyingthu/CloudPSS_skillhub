...
class TestPowerFlowUnit:
    ...
    def test_run_fails_when_result_tables_have_no_rows(
        self, mock_model_utils_model, mock_model_class, mock_set_token, tmp_path
    ):
        ...
        job.result.getBuses.return_value = [
            {"type": "table", "data": {"columns": [{"name": "Vm", "data": []}]}}
        ]
        job.result.getBranches.return_value = [
            {"type": "table", "data": {"columns": [{"name": "Branch", "data": []}]}}
        ]
        model.runPowerFlow.return_value = job
        mock_model_class.fetch.return_value = model
        mock_model_class.load.return_value = model
        mock_model_utils_model.fetch.return_value = model
        mock_model_utils_model.load.return_value = model
