...
class TestDUDVCurveLive:
    """真实 CloudPSS 集成测试 - 验证DUDV曲线"""
    ...
    def test_skill_config_requires_model(self):
        ...
        config = {
            "input": {"result_file": "./results.json"}
        }
    ...
    def test_skill_name_is_correct(self):
        """验证技能名称正确"""
        skill = DUDVCurveSkill()
        assert skill.name == "dudv_curve", f"技能名称应该是 dudv_curve: {skill.name}"
    ...
    def test_skill_description_matches_functionality(self):
        ...
        keywords = ["dudv", "du/dv", "curve", "曲线", "电压"]
        assert any(kw in desc for kw in keywords), f"描述应该包含DUDV曲线相关词汇: {desc}"
