...
def test_param_scan_emt():
    ...
    # 使用较小的变化范围
    values = [base_value * 0.95, base_value * 0.98, base_value, base_value * 1.02, base_value * 1.05]
    ...
    for i, value in enumerate(values):
        ...
        try:
            model.updateComponent(
                target_comp,
                args={target_param: {"source": str(value), "ɵexp": ""}}
            )
            print(f"      ✓ 已设置参数")
    ...
    # 保存结果
    result_file = f"{RESULTS_DIR}/param_scan_emt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
