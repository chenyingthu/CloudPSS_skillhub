...
class DUDVCurveSkill(SkillBase):
            ...
            "properties": {
                ...
                "input": {
                    ...
                    "properties": {
                        "result_file": {
                            "type": "string",
                            "description": "disturbance_severity 结果JSON文件",
                        },
                    },
                },
                "buses": {
                    ...
                    "items": {"type": "string", "description": "母线label，如Bus_16"},
                },
                "simulation": {
                    "type": "object",
                    "properties": {
                ...
                "dudv": {
                    ...
                    "properties": {
                        ...
                        "injection_bus": {
                            "type": "string",
                            "description": "无功注入母线 (可选，默认为被分析母线)",
                        },
                        "injection_duration": {
                            "type": "number",
                            "default": 2.0,
                            "description": "每次无功注入持续时间",
                        },
                    },
                },
                "output": {
    ...
    def run(self, config: Dict[str, Any]) -> SkillResult:
        ...
        try:
            ...
            logger.info(
                f"DUDV曲线分析开始 - 结果文件: {result_file}, 母线数: {len(bus_labels)}"
            )
            logs.append(
                LogEntry(
                    timestamp=datetime.now(),
                    level="INFO",
                    message=f"DUDV分析开始，母线: {bus_labels}",
                )
            )

            dudv_results = self.from_disturbance_severity_result(
                result_file, bus_labels
            )
            if not dudv_results:
                raise RuntimeError(
                    "未从 disturbance_severity 结果文件中提取到任何 DUDV 数据"
                )
            ...
            logs.append(
                LogEntry(
                    timestamp=datetime.now(),
                    level="INFO",
                    message=f"DUDV分析完成，耗时: {duration:.2f}s",
                )
            )
            ...
            return SkillResult(
                ...
                data={
                    ...
                    "num_points": max(
                        len(item.get("voltage", [])) for item in dudv_results.values()
                    )
                    if dudv_results
                    else 0,
                },
    ...
    def from_disturbance_severity_result(
        result_file: str, bus_labels: Optional[List[str]] = None
    ) -> Dict[str, List[float]]:
        """
        从扰动严重度分析结果加载DUDV数据
        ...
        for bus_result in channel_results:
            ...
            if v_steady is not None:
                ...
                dudv_data[label] = {
                    ...
                    "dv": [
                        dv_down if dv_down != 0 else -0.1,
                        0,
                        dv_up if dv_up != 0 else 0.1,
                    ],
                }
