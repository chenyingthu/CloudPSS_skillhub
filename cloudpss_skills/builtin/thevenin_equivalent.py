...
class TheveninEquivalentSkill(SkillBase):
        ...
        return {
            ...
            "properties": {
                ...
                "pcc": {
                    ...
                    "required": ["bus"],
                    "properties": {
                        "bus": {"type": "string", "description": "PCC母线名，如 bus8 / Bus8 / BUS_8"},
                    },
                },
                "equivalent": {
                    "type": "object",
                    "properties": {
                        ...
                        "rating_mva": {"type": "number", "description": "可选。若提供则顺带计算SCR"},
                    },
                },
                "output": {
                    "type": "object",
    ...
    def run(self, config: Dict[str, Any]) -> SkillResult:
        ...
        try:
            ...
            system_base_mva = float(config.get("equivalent", {}).get("system_base_mva", 100.0))
            rating_mva = config.get("equivalent", {}).get("rating_mva")

            log("INFO", f"计算PCC戴维南等值: model={model.rid}, bus={pcc_bus}")
            zth = compute_positive_sequence_zth(model, pcc_bus, system_base_mva=system_base_mva)
            ...
            if rating_mva is not None:
                scr = short_circuit_capacity_mva / float(rating_mva) if float(rating_mva) > 0 else float("inf")
            ...
            filepath.write_text(json.dumps(result_data, ensure_ascii=False, indent=2), encoding="utf-8")
            artifacts.append(Artifact(type="json", path=str(filepath), size=filepath.stat().st_size, description="PCC戴维南等值结果"))

            log("INFO", f"Zth={result_data['z_th_pu']}, Ssc={result_data['short_circuit_capacity_mva']} MVA")
            return SkillResult(
