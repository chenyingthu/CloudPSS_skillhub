...
class HDF5ExportSkill(SkillBase):
            ...
            "properties": {
                "source": {
                    ...
                    "properties": {
                        ...
                        "rid": {
                            "type": "string",
                            "description": "仿真结果RID（cloud类型）",
                        },
                        "file_path": {
                            "type": "string",
                            "description": "结果文件路径（file类型）",
                        },
                        "data": {
                            "type": "object",
                            "description": "内存数据（memory类型）",
                        },
                    },
                },
                "output": {
        ...
        try:
            ...
            logs.append(
                LogEntry(
                    ...
                    level="INFO",
                    message=f"HDF5导出开始，源类型: {source_type}",
                )
            )
            ...
            logs.append(
                LogEntry(
                    timestamp=datetime.now(),
                    level="INFO",
                    message=f"HDF5导出完成，文件: {hdf5_path}",
                )
            )
    ...
    def _export_to_hdf5(
        ...
        with h5py.File(hdf5_path, "w") as f:
            ...
            if data_type == "emt":
                self._export_emt_data(
                    f, data, compression, compression_level, chunk_size, options
                )
    ...
    @staticmethod
    def read_hdf5(
        hdf5_path: str, dataset_path: Optional[str] = None
