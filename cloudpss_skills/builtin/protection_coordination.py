...
class ProtectionCoordinationSkill(SkillBase):
    ...
    def run(self, config: Dict) -> SkillResult:
        ...
        try:
            ...
            if analysis_config.get("distance_protection", {}).get("enabled", True):
                logger.info("分析距离保护...")
                self._analyze_distance_protection(
                    analysis_config.get("distance_protection", {})
                )

            if analysis_config.get("overcurrent_protection", {}).get("enabled", True):
                logger.info("分析过流保护...")
                self._analyze_overcurrent_protection(
                    analysis_config.get("overcurrent_protection", {})
                )
            ...
            result_data = {
                ...
                "distance_relays": len(
                    [
                        r
                        for r in self.protection_devices.values()
                        if r.relay_type == ProtectionType.DISTANCE
                    ]
                ),
                "overcurrent_relays": len(
                    [
                        r
                        for r in self.protection_devices.values()
    ...
    def _analyze_distance_protection(self, config: Dict):
        ...
        for relay in distance_relays:
            relay_analysis = {"location": relay.location, "zones": {}}
    ...
    def _analyze_differential_protection(self):
        ...
        for relay in diff_relays:
            if (
                "T1" in relay.location
                or "T2" in relay.location
                or "T3" in relay.location
            ):
            ...
            scenario_results.append(
                {
                    ...
                    "expected_clearing_time": self._estimate_clearing_time(
                        operating_relays
                    ),
                }
            )
    ...
    def _determine_operating_relays(self, location: str, fault_type: str) -> List[str]:
        ...
        for key, relay in self.protection_devices.items():
            if location.lower() in relay.location.lower():
                ...
                fault_type == "single_ground"
                and relay.relay_type == ProtectionType.ZERO_SEQUENCE
            ):
    ...
    def _generate_tcc_curves(self):
        """生成时间-电流配合曲线数据"""
        tcc_data = {"curves": [], "coordination_points": []}
        ...
        for relay in oc_relays[:3]:  # 取前3个作为示例
            ...
            for current_mult in [1.5, 2, 3, 5, 10, 20]:
                ...
                curve_points.append(
                    {"current_multiplier": current_mult, "time_s": time}
                )
