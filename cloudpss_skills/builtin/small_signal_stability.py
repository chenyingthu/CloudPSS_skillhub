...
class SmallSignalStabilitySkill(SkillBase):
                ...
                "analysis": {
                    ...
                    "properties": {
                        ...
                        "freq_range": {
                            ...
                            "maxItems": 2,
                            "default": [0.1, 2.0],
                            "description": "机电振荡频率范围[Hz]，必须为2元素数组[min, max]",
                        },
                        "base_power": {
                            "type": "number",
                            "default": 100.0,
    ...
    def run(self, config: Dict[str, Any]) -> SkillResult:
        ...
        try:
            ...
            log(
                "INFO",
                f"阻尼阈值: {damping_threshold}, 频率范围: {freq_range[0]}-{freq_range[1]} Hz",
            )
            ...
            eigen_results = self._eigenvalue_analysis(
                state_matrix, freq_range, damping_threshold, log
            )

            # 计算参与因子
            log("INFO", "计算参与因子...")
            participation_factors = self._calculate_participation_factors(
                state_matrix, eigen_results, system_data, log
            )
    ...
    def _extract_system_data(
        self, model, powerflow_result, base_power: float, log_func
    ) -> Dict:
                    ...
                    buses.append(
                        {
                            ...
                            "angle_deg": float(bus_angles[i])
                            if i < len(bus_angles) and bus_angles[i]
                            else 0.0,
        ...
        for key, comp in components.items():
            ...
            if (
                "EXST1" in comp_def
                or "_EXST1" in comp_def
                or "exciter" in comp_def.lower()
            ):
            ...
            if (
                "GOV" in comp_def
                or "_STEAM_GOV" in comp_def
                or "governor" in comp_def.lower()
            ):
        ...
        for key, comp in components.items():
            ...
            if (
                "Generator" in comp_def
                or "_newGenerator" in comp_def
                or "_newSyncMachine" in comp_def
            ):
                gen_data = {
                    "key": key,
                    "label": comp_label,
        ...
        log_func(
            "INFO",
            f"  励磁系统: {len(exciters)} 个, PSS: {len(pss_systems)} 个, 调速器: {len(governors)} 个",
        )
        ...
        for i, gen_i in enumerate(generators):
            ...
            for j, gen_j in enumerate(generators):
                if i != j:
                    ...
                    Xdp_sum = Xdp_i + Xdp_j
                    K_ij = (
                        omega_s / (2 * H_i) * power_coupling / Xdp_sum * 0.005
                    )  # 减小耦合系数
            ...
            log_func(
                "DEBUG",
                f"  发电机 {i + 1} ({gen_i.get('label', 'N/A')}): "
                f"H={H_i:.2f}s, Xdp={Xdp_i:.3f}, P={P_i:.1f}MW, "
                f"D={D_i:.2f}, V={V_i:.3f}",
            )

        log_func(
            "INFO", f"改进状态矩阵维度: {n_states}x{n_states} (每台机2状态，含详细参数)"
        )
    ...
    def _eigenvalue_analysis(
        ...
        for i, ev in enumerate(eigenvalues):
            ...
            if real_part < 0:
                time_constant = (
                    -1.0 / real_part if abs(real_part) > 1e-10 else float("inf")
                )
            else:
            ...
            mode_info = {
                ...
                "status": "stable"
                if damping_ratio > 0 and real_part < 0
                else "unstable",
            }
        ...
        assessment = {
            "n_modes": len(modes),
            "n_electromechanical": sum(
                1 for m in modes if m.get("is_electromechanical")
            ),
        ...
        for mode in modes[:5]:
            if mode.get("is_electromechanical"):
                log_func(
                    "INFO",
                    f"  模式 {mode['index']}: f={mode['frequency']:.3f}Hz, "
    ...
    def _calculate_participation_factors(
        self, state_matrix: np.ndarray, eigen_results: Dict, system_data: Dict, log_func
    ) -> Dict:
        """计算参与因子 - 2状态模型（δ, ω）"""
        generators = system_data["generators"]
        eigenvalues = eigen_results["eigenvalues"]
            ...
            for mode_idx, ev in enumerate(eigenvalues):
                ...
                if isinstance(ev, dict):
                    ...
                    imag_val = (
                        ...
                        and "complex" in str(type(ev)).lower()
                        else 0.0
                    )
                ...
                for i in range(n_states):
                    # 归一化参与因子
                    if (
                        mode_idx < right_eigenvectors.shape[1]
                        and mode_idx < left_eigenvectors.shape[1]
                    ):
                        p_ik = abs(
                            left_eigenvectors[i, mode_idx]
                            * right_eigenvectors[i, mode_idx]
                        )
                ...
                if mode_participation:
                    max_p = (
                        max(mode_participation) if max(mode_participation) > 0 else 1.0
                    )
                    ...
                    for gen_idx, gen in enumerate(generators):
                        ...
                        delta_p = (
                            normalized_p[delta_idx]
                            if delta_idx < len(normalized_p)
                            else 0
                        )
                        omega_p = (
                            normalized_p[omega_idx]
                            if omega_idx < len(normalized_p)
                            else 0
                        )
                    ...
                    if mode_idx < len(eigen_results["modes"]):
                        eigen_results["modes"][mode_idx]["dominant_gens"] = [
                            g["gen"] for g in dominant_gens[:3]
                        ]

            log_func(
                "INFO", f"参与因子计算完成，分析了 {len(participation)} 个机电模式"
            )
    ...
    def _generate_report(self, data: Dict, path: Path):
        ...
        for mode in modes:
            if mode.get("is_electromechanical"):
                ...
                lines.append(
                    f"| {mode['index']} | {freq:.3f} | {damping:.3f} | {status} | {dominant} |"
                )
