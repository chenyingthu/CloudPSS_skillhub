...
from cloudpss_skills.core.auth_utils import fetch_model_by_rid, run_emt, run_powerflow
...
class BatchTaskManagerSkill(SkillBase):
    ...
    def run(self, config: Dict[str, Any]) -> SkillResult:
        ...
        try:
            ...
            logs.append(
                LogEntry(
                    level="INFO",
                    message=f"批量任务管理开始，共{len(tasks_config)}个任务",
                )
            )
            ...
            result_data = {
                ...
                "tasks_per_second": result.total_tasks / result.execution_time
            ...
            return SkillResult(
                ...
                metrics={
                    ...
                    "completed_tasks": result.completed_tasks,
                    "failed_tasks": result.failed_tasks,
                    "tasks_per_second": result.total_tasks / duration
                    if duration > 0
                    else 0,
                },
            )
    ...
    async def _run_parallel(
        ...
        async def run_with_semaphore(task: BatchTask):
            async with semaphore:
                await self._execute_task(
                    task, config, polling_interval, timeout, enable_retry
                )
        ...
        result.cancelled_tasks = sum(
            1 for t in tasks if t.status == TaskStatus.CANCELLED
        )
        ...
        while task.retry_count <= task.max_retries:
            try:
                ...
                if task.task_type == "emt":
                    job = await self._run_emt_task(model, config)
                elif task.task_type == "power_flow":
                    job = await self._run_power_flow_task(model, config)
                else:
                ...
                if task.retry_count > task.max_retries or not enable_retry:
                    ...
                    logger.warning(
                        f"任务 {task.name} 失败，第{task.retry_count}次重试: {e}"
                    )
                    await asyncio.sleep(5)  # 重试前等待

    async def _run_emt_task(self, model, config: Dict[str, Any]) -> Any:
        """运行EMT任务"""
        job = run_emt(model, config)
        return job

    async def _run_power_flow_task(self, model, config: Dict[str, Any]) -> Any:
        """运行潮流任务"""
    ...
    @staticmethod
    def create_n1_tasks(
        model_rid: str, bus_labels: List[str], line_keys: List[str]
    ) -> List[Dict]:
        """创建N-1分析任务列表"""
        tasks = []
